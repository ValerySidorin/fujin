...
impl InheritedListeners {
    ...
    #[cfg(unix)]
    fn from_entries(entries: Vec<(ListenerMetadata, std::os::fd::OwnedFd)>) -> Self {
        Self {
...
impl ListenerRegistry {
    ...
    /// # Errors
    ///
    /// Returns [`UpgradeError::DuplicateListener`] when the descriptor key is already present.
    #[cfg(unix)]
    pub fn register(
        &self,
        metadata: ListenerMetadata,
        ...
        if entries.contains_key(&key) {
            return Err(UpgradeError::DuplicateListener(key));
        }
    ...
    #[cfg(unix)]
    fn snapshot(&self) -> Result<Vec<(ListenerMetadata, std::os::fd::OwnedFd)>, UpgradeError> {
        use std::os::fd::AsFd;

        let entries = self.entries.lock();
        if entries.len() != self.expected {
            return Err(UpgradeError::ListenersNotReady {

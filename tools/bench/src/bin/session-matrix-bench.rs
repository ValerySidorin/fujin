...
use fujin_connector::{AcceptanceGuarantee, Catalog};
use fujin_middleware::NoBindMiddleware;
use fujin_native::{RequestCode, ResponseCode};
use fujin_transport::{TransportConfig, TransportRegistry};
use fujin_transport_websocket::NativeWebSocketStream;
...
impl ServerHandle {
    async fn start(transport: Transport, catalog: Arc<Catalog>) -> Result<Self> {
        #[cfg(unix)]
        ...
        let transport_config = match transport {
            Transport::Tcp => {
                registry.register(fujin_transport_tcp::plugin())?;
                TransportConfig {
            ...
            Transport::Unix => {
                #[cfg(unix)]
                {
                    registry.register(fujin_transport_unix::plugin())?;
                    TransportConfig {
            ...
            Transport::WebSocket => {
                registry.register(fujin_transport_websocket::plugin())?;
                TransportConfig {
            ...
            Transport::Quic => {
                ...
                registry.register(fujin_transport_quic::plugin())?;
                TransportConfig {
        ...
        let config = fujin_runtime::ServerConfig {
        ...
        let task = tokio::spawn(async move {
            fujin_runtime::serve(config, catalog, Arc::new(NoBindMiddleware), server_shutdown).await
        });
        Ok(Self {
            shutdown,
            task,

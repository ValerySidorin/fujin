...
pub trait BindMiddleware: Send + Sync + 'static {
    ...
    async fn process(&self, context: &mut BindContext<'_>) -> Result<()>;
}
...
impl BindMiddlewareRunner for BindMiddlewareRegistry {
    async fn run(
            ...
            .filter(|entry| entry.enabled)
        {
            ...
            plugin.process(&config.settings, &mut context).await?;
        }
        Ok(())
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn bind_registry_runs_enabled_plugins_in_configuration_order() {

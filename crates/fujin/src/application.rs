...
struct UpgradeSetup {
    client: Option<fujin_runtime::upgrade::UpgradeClient>,
...
async fn prepare_upgrade(
    ...
    let client = fujin_runtime::upgrade::request_from_environment()
        .await
    ...
    let inherited = client.as_ref().map_or_else(
        ...
        fujin_runtime::upgrade::UpgradeClient::inherited,
    );
    let registry = ListenerRegistry::new(fujin_runtime::configured_listener_count(server_config));
    let socket = fujin_runtime::upgrade::socket_path_from_environment();
...
async fn complete_upgrade(
    client: fujin_runtime::upgrade::UpgradeClient,
    socket: &str,
    registry: &ListenerRegistry,
    upgrade_shutdown: &CancellationToken,
    shutdown_requested: &CancellationToken,
    ...
    fujin_runtime::upgrade::wait_for_socket_release(socket)
        .await
...
fn spawn_upgrade_listener(
    ...
    tokio::spawn(async move {
        let result = fujin_runtime::upgrade::listen_for_upgrade(
            socket_path,
            registry,
            shutdown.clone(),
            drain,

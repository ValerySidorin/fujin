...
use crate::{ApplicationBuilder, ApplicationHandle};

/// Builds and runs a command-line Fujin application until shutdown.
///
/// Generated composition binaries use this entry point so logging, signal handling, connector
/// reload, and version output remain identical to the standard `fujin-app` binary.
///
/// # Errors
///
/// Returns bootstrap, listener, signal, reload, or shutdown failures.
pub async fn run_cli(builder: ApplicationBuilder, build_version: &'static str) -> Result<()> {
    let log_filter = configure_logging()?;
    if parse_arguments(build_version)? {
        return Ok(());
    }

    let application = builder.build().await.context("build Fujin application")?;
...
fn parse_arguments(build_version: &str) -> Result<bool> {
...
mod tests {
    ...
    #[test]
    fn logging_level_matches_environment_contract() {

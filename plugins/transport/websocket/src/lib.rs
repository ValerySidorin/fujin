                ...
                accepted = listener.accept() => {
                    ...
                    sessions.spawn(async move {
                        if let Some(tls) = tls {
                            ...
                            websocket_session(stream, session, policy, native).await
                        } else {
                            websocket_session(stream, session, policy, native).await
...
async fn websocket_session<S>(
    ...
    native: fujin_transport::NativeSessionConfig,
) -> Result<()>
where
    S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    context
        .serve_native_stream_with_config(NativeWebSocketStream::new(websocket), native)
        .await
}

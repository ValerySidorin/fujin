...
func compileConnector(raw any) (connector.Compiled, error) {
	...
	if parsed, ok := raw.(Config); ok {
		...
		return nil, fmt.Errorf("nats_core connector: convert config: %w", err)
	}
	...
	for route, settings := range config.Routes {
		...
		profiles[route] = connector.RouteProfile{Produce: true, Subscribe: true, ProduceGuarantee: connector.AcceptanceLocal}

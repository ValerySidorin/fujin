...
func compileConnector(raw any) (connector.Compiled, error) {
	...
	if parsed, ok := raw.(Config); ok {
		...
		return nil, fmt.Errorf("nsq connector: convert config: %w", err)
	}
	...
	for route, settings := range config.Routes {
		...
		if err := conf.ValidateReader(); err == nil {
			...
			profile.Settlement = connector.SettlementProfile{Ack: connector.AckSingle, Nack: connector.NackRequeue}
